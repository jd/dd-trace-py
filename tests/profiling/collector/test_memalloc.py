...
def test_get_aggregated_events():
    _memalloc.start(32, 10000)
    _allocate_1k()
    events = _memalloc.get_aggregated_events()
    _memalloc.stop()

    for ((stack, nframe, thread_id), (count, size)) in events.items():
        assert 0 < len(stack) <= 32
        ...
        if last_call[2] == "_allocate_1k" and last_call[1] == _ALLOC_LINE_NUMBER:
            assert count == 1000
            assert size > 1000  # size depends on the object size
            assert last_call[0] == __file__
            assert stack[1][0] == __file__
            assert stack[1][1] == 54
            assert stack[1][2] == "test_get_aggregated_events"
            break
    else:
        pytest.fail("Allocation not found")
...
def test_get_aggregated_events_truncate():
    max_nframe = 2
    ...
    for ((stack, nframe, thread_id), (count, size)) in events.items():
        ...
        if last_call[2] == "_allocate_1k" and last_call[1] == _ALLOC_LINE_NUMBER:
            ...
            assert stack[1][0] == __file__
            assert stack[1][1] == 76
            assert stack[1][2] == "_pre_allocate_1k"
            break
    else:
        pytest.fail("Allocation not found")
...
def test_get_aggregated__events_not_started():
    with pytest.raises(RuntimeError, match="the memalloc module was not started"):
        _memalloc.get_aggregated_events()
