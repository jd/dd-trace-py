...
PyDoc_STRVAR(memalloc_get_aggregated_events__doc__, "get_aggregated_events($module, /)\n"
                                                    "--\n"
                                                    "\n"
                                                    "Get memory allocations events aggregated by stack traces.\n"
                                                    "\n"
                                                    "Also clear traces of memory blocks allocated by Python.");
static PyObject*
memalloc_get_aggregated_events(PyObject* Py_UNUSED(module), PyObject* Py_UNUSED(ignored))
{
    ...
    /* Reset the traceback list right away.
       The code below allocates Python memory that will be tracked. */
    traceback_list_t* prev_tb_list = global_traceback_list;
    global_traceback_list = traceback_list_new();

    for (uint32_t i = 0; i < prev_tb_list->count; i++) {
        traceback_t* tb = prev_tb_list->tracebacks[i];

        /* Convert stack into a tuple of tuple */
        PyObject* stack_and_total_nframe = traceback_to_tuple(tb, false);

        PyObject* count_obj;
        PyObject* size_obj = PyLong_FromSize_t(tb->size);
        PyObject* existing_data = PyDict_GetItem(events, stack_and_total_nframe);

        if (existing_data) {
            PyObject *existing_size, *existing_count;

            existing_count = PyTuple_GET_ITEM(existing_data, 0);
            existing_size = PyTuple_GET_ITEM(existing_data, 1);

            count_obj = PyNumber_Add(existing_count, number_one);
            /* FIXME: handle existing_count == NULL (error) */
            PyObject* previous_size_obj = size_obj;
            size_obj = PyNumber_Add(existing_size, previous_size_obj);
            /* FIXME: handle existing_size == NULL (error) */

            Py_DECREF(previous_size_obj);
        } else
            count_obj = PyLong_FromLong(1);

        PyObject* count_and_size = PyTuple_New(2);

        PyTuple_SET_ITEM(count_and_size, 0, count_obj);
        PyTuple_SET_ITEM(count_and_size, 1, size_obj);
        /* Use PyDict_SetItem as we might be replacing data  */
        PyDict_SetItem(events, stack_and_total_nframe, count_and_size);

        Py_DECREF(stack_and_total_nframe);
        Py_DECREF(count_and_size);

        /* Free the traceback */
        PyMem_RawFree(tb);
    }

    traceback_list_free(prev_tb_list);
...
static PyMethodDef module_methods[] = { { "start", (PyCFunction)memalloc_start, METH_VARARGS, memalloc_start__doc__ },
                                        { "get_aggregated_events", (PyCFunction)memalloc_get_aggregated_events,
                                          METH_NOARGS, memalloc_get_aggregated_events__doc__ },
